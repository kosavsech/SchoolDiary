...
internal class SegmentCoordinatesComputer {
	...
	fun segmentCoordinates(
		...
		val segmentWidth =
			(width - (segmentSpacing + segmentTangent) * (segmentCount - 1)) / segmentCount
		val isLast = position == segmentCount - 1
		
		val topLeft = (segmentWidth + segmentTangent + segmentSpacing) * position
		val bottomLeft =
			(segmentWidth + segmentSpacing) * position + segmentTangent * max(0, position - 1)
		val topRight =
			(segmentWidth + segmentTangent) * (position + 1) + segmentSpacing * position - if (isLast) segmentTangent else 0f
		val bottomRight =
			segmentWidth * (position + 1) + (segmentTangent + segmentSpacing) * position
	...
	fun progressCoordinates(
		...
		val lastSegmentOffset =
			if (isLastSegment) segmentTangent * (1 - (segmentCount - progress)) else 0f
		
		val topRight =
			(segmentWidth + segmentTangent) * progress + segmentSpacing * spacingCount - lastSegmentOffset
